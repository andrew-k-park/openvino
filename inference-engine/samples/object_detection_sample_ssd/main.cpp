...
int main(int argc, char* argv[]) {
    try {
        ...
        // -----------------------------------------------------------------------------------------------------
        auto num_threads = FLAGS_nthreads;
        auto threads = std::vector<std::thread>();
        for (auto i = 0; i < num_threads; ++i) {
            threads.emplace_back([&]() {
                // --------------------------- Step 4. Loading model to the device
                // ------------------------------------------
                slog::info << "[Thread=" << std::this_thread::get_id() << "] Loading model to the device" << slog::endl;
                ...
                // -------------------------------------------------
                slog::info << "[Thread=" << std::this_thread::get_id() << "] Create infer request" << slog::endl;
                ...
                slog::info << "[Thread=" << std::this_thread::get_id() << "] Batch size is " << std::to_string(batchSize) << slog::endl;
                ...
                // --------------------------- Step 7. Do inference
                // ---------------------------------------------------------
                slog::info << "[Thread=" << std::this_thread::get_id() << "] Start inference" << slog::endl;
                infer_request.Infer();
                // -----------------------------------------------------------------------------------------------------

                // --------------------------- Step 8. Process output
                // -------------------------------------------------------
                slog::info << "[Thread=" << std::this_thread::get_id() << "] Processing output blobs" << slog::endl;
                ...
                for (int curProposal = 0; curProposal < maxProposalCount; curProposal++) {
                    ...
                    slog::info << "[Thread=" << std::this_thread::get_id() << "] [" << curProposal << "," << label << "] element, prob = " << confidence << "    (" << xmin
                               << "," << ymin << ")-(" << xmax << "," << ymax << ")"
                               << " batch id : " << image_id;
                ...
                for (size_t batch_id = 0; batch_id < batchSize; ++batch_id) {
                    ...
                    const std::string image_path = "out_" + ss.str() + "_" + std::to_string(batch_id) + ".bmp";
