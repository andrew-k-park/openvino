...
public:
    void prepare_controlled_data(PagedAttentionManager& pam, const paged_attention_test_params& p) {
...
class paged_attention_u4_mixed_micro_swa_test : public paged_attention_u4_mixed_ocl_test {};

TEST_P(paged_attention_u4_mixed_micro_swa_test, matches_cpu_reference) {
    if (!tests::get_test_engine().get_device_info().supports_immad)
    ...
    if (result.network->get_config().get_enable_u4_mixed_ocl_fallback())
        GTEST_SKIP() << "Set OV_GPU_ENABLE_U4_MIXED_OCL_FALLBACK=0 to exercise U4 MIXED micro-SDPA SWA";
    ...
    ASSERT_NE(impl, nullptr);
    const auto dump_info = impl->get_kernels_dump_info(*pa_inst->get_impl_params());
    EXPECT_NE(dump_info.get_entries().find("sdpa_micro__generate"), std::string::npos)
        << "Disabling the U4 BY_CHANNEL MIXED fallback must restore micro SDPA: " << dump_info.get_entries();
    EXPECT_EQ(dump_info.get_entries().find("paged_attention_opt__multi_tokens"), std::string::npos)
        << "Disabling the U4 BY_CHANNEL MIXED fallback must not exercise OCL PagedAttention: "
...
INSTANTIATE_TEST_SUITE_P(
    regression_paged_attention_u4_mixed_micro_swa,
    paged_attention_u4_mixed_micro_swa_test,
    ::testing::Values(
        paged_attention_test_params{{{8, 7}}, 32, 2, 128, 128, 16, 5, ENABLE_CACHE_COMPRESSION, ov::internal::CacheQuantMode::BY_CHANNEL, DYNAMIC_INPUT_PAD, DISABLE_SCORES, DISABLE_ROTATION, DISABLE_FA_V2, false, 0, {}, false, {}, {}, ov::element::u4},
        paged_attention_test_params{{{120, 120}}, 32, 2, 128, 128, 16, 5, ENABLE_CACHE_COMPRESSION, ov::internal::CacheQuantMode::BY_CHANNEL, DYNAMIC_INPUT_PAD, DISABLE_SCORES, DISABLE_ROTATION, DISABLE_FA_V2, false, 0, {}, false, {}, {}, ov::element::u4}));
