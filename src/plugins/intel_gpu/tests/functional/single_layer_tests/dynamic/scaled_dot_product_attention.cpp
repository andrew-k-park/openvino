...
class ScaledAttnScalarBooleanMaskGPUTest : public testing::Test {
protected:
    void SetUp() override {
...
TEST_F(ScaledAttnScalarBooleanMaskGPUTest, ConstantMatchesReference) {
    auto make_model = [](bool mask_value) {
        auto query = std::make_shared<ov::op::v0::Parameter>(ov::element::f32, ov::Shape{1, 1, 1, 2});
        auto key = std::make_shared<ov::op::v0::Parameter>(ov::element::f32, ov::Shape{1, 1, 2, 2});
        auto value = std::make_shared<ov::op::v0::Parameter>(ov::element::f32, ov::Shape{1, 1, 2, 2});
        ov::ParameterVector parameters{query, key, value};
        auto mask = std::make_shared<ov::op::v0::Constant>(ov::element::boolean, ov::Shape{}, mask_value);
    ...
    const std::vector<float> query_values{4.0f, 0.0f};
    const std::vector<float> key_values{4.0f, 0.0f, -4.0f, 0.0f};
    const std::vector<float> value_values{2.0f, 4.0f, 6.0f, 8.0f};
    const std::array<std::vector<float>, 2> expected_outputs{{{4.0f, 6.0f}, {2.0f, 4.0f}}};

    for (const bool mask_value : {false, true}) {
        auto model = make_model(mask_value);
        ...
        for (size_t input_idx = 0; input_idx < input_values.size(); ++input_idx) {
            ...
            ov::Tensor input_tensor(ov::element::f32, model->input(input_idx).get_shape());
            std::copy(values.begin(), values.end(), input_tensor.data<float>());
            request.set_tensor(compiled_model.input(input_idx), input_tensor);
        }

        request.infer();
        const auto gpu_output = request.get_output_tensor();
        const auto& expected_output = expected_outputs[mask_value];
        ASSERT_EQ(gpu_output.get_size(), expected_output.size());
        for (size_t output_idx = 0; output_idx < gpu_output.get_size(); ++output_idx) {
            const auto actual = gpu_output.data<float>()[output_idx];
