...
bool primitive_inst::set_output_layout(layout& new_layout, bool invalidate_users_if_changed) {
    merge_output_padding(new_layout.data_padding);
    new_layout.data_padding = _output_layout.data_padding;
    bool changed = (new_layout != _output_layout);
    if (changed && invalidate_users_if_changed)
        invalidate_users();

    _output_layout = new_layout;
    _valid_output_layout = true;
    return changed;
}

void primitive_inst::invalidate_users() const {
    for (const auto& user : _node.get_users()) {
        const auto prim = _network.get_primitive(user->id());
        if (prim->_valid_output_layout) {
            prim->_valid_output_layout = false;
            prim->invalidate_users();
        }
    }
}
...
primitive_inst::primitive_inst(network& network, program_node const& node, bool allocate_memory)
    : _network(network), _node(node), _impl(node.get_selected_impl() ? node.get_selected_impl()->clone() : nullptr),
      _output(), _output_layout(node.get_output_layout()), _valid_output_layout(true), _output_changed(false),
      _mem_allocated(allocate_memory), max_output_layout_size(0) {
    if (allocate_memory) {
...
memory::ptr primitive_inst::allocate_output(engine& _engine, network& _network, memory_pool& pool, const program_node& _node,
        const layout& _layout, bool is_internal) {
    auto get_memory_from_pool = [&](engine& _engine, const layout& layout, const primitive_id id, std::set<primitive_id> dependencies,
            allocation_type type, bool reusable) {
        if (_engine.configuration().use_memory_pool)
                return pool.get_memory(layout, id, 0, dependencies, type, reusable);
    ...
    bool usm_device_allocatable = true;
    const auto& total_device_input_mem_size = std::accumulate(inst_deps.begin(), inst_deps.end(), (uint64_t)0, device_mem_acc);
    if (total_device_input_mem_size > _engine.get_device_info().max_global_mem_size)
        usm_device_allocatable = false;
    ...
    GPU_DEBUG_GET_INSTANCE(debug_config);
    const auto& lockable_mem_type = _engine.get_lockable_preffered_memory_allocation_type(_layout.format.is_image_2d());
    const auto& alloc_type = use_lockable_memory ? lockable_mem_type
        : usm_device_allocatable ? allocation_type::usm_device : lockable_mem_type;
    ...
    if (is_internal && (_node.can_be_optimized() || _node.is_type<generic_layer>())) {
        ...
        return _engine.allocate_memory(_layout, allocation_type::usm_device, false);
    } else if (is_internal && !_node.is_output() && _node.is_type<input_layout>()) {
        // Skip memory reset for input_layout primitives, since data will be copied from cldnn::data primitive
        // or just reuse primitive's memory
        GPU_DEBUG_IF(debug_config->verbose >= 2) {
        ...
        return _engine.allocate_memory(_layout, alloc_type, false);
    } else if (is_internal || (!_node.can_share_buffer()) || _node.can_be_optimized() || _node.is_output()) {
        GPU_DEBUG_IF(debug_config->verbose >= 2) {
            GPU_DEBUG_COUT << "[" << _node.id() << ": output]" << std::endl;
        }
        return _engine.allocate_memory(_layout, alloc_type);
    } else {
        return get_memory_from_pool(_engine,
                _layout,
                _node.id(),
...
memory::ptr primitive_inst::allocate_output() {
    return allocate_output(get_network().get_engine(), _network, _network.get_memory_pool(), _node, _output_layout, _network.is_internal());
}
