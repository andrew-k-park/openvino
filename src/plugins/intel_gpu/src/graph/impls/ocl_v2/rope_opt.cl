...
#ifdef LTX_VIDEO
// LTX-Video specific 3D spatial-temporal RoPE
// Input: [batch, seq_len, 2048] - flat tensor, no head structure
// Cos/Sin: [batch, seq_len, 2048] - pre-computed from 3D grid (frames, height, width)
// Complex interleaved format: [real0, imag0, real1, imag1, ..., real1023, imag1023]
KERNEL(rope_opt)(
    OPTIONAL_SHAPE_INFO_ARG const __global INPUT0_TYPE* input,
    const __global INPUT1_TYPE* cos,
    const __global INPUT2_TYPE* sin,
    __global OUTPUT_TYPE* output) {
#if VEC_SIZE != 1 && VEC_SIZE != 8 && VEC_SIZE != 16
#   error "rope_opt.cl (LTX_VIDEO) - VEC_SIZE must be one of {1, 8, 16}"
#endif

    // Global work dimensions:
    // get_global_id(0): batch index
    // get_global_id(1): sequence position index
    // get_global_id(2): vector index within 2048 dimensions
    const uint b = get_global_id(0);
    const uint p = get_global_id(1);
