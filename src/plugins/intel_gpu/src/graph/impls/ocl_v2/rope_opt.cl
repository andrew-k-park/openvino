...
#   error "rope_opt.cl - 2, 3 or 4 of INPUT1_DIMS/INPUT2_DIMS is supported only"
    ...
    // Read interleaved lanes and de-interleave into even (in1) / odd (in2), exactly as RotateInterleaved
    // loads its input; the rotation and the half-split write below stay as in plain RotateHalf. This is
    // the kernel-side equivalent of an even/odd gather: it lets the llama.cpp NORMAL RoPE
    // (interleaved-in, half-split-out) run without an explicit gather/concat in the graph. r is doubled
    // because the two VEC_SIZE-wide loads span the 2*VEC_SIZE interleaved elements behind VEC_SIZE pairs.
