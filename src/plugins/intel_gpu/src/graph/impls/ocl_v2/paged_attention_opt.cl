...
KERNEL(pa_sdpa_opt)(
    ...
    // Only compute for the last head to avoid redundant calculations
    if (head_num_idx == 0 && partition_idx == 0) {
        const int start_size = adaptive_rkv_start_size[0];
        const int evictable_size = adaptive_rkv_evictable_sizes[subsequence_idx];
        
        if (evictable_size > 0 && seq_len >= start_size + evictable_size) {
            // This is a simplified inline version - full implementation would be in separate kernel
            // Mark that diversity calculation is needed by writing a flag or calling separate kernel
        }
    }
#endif
}
...
// ============================================================================
// SDPA_STAGE_3: Adaptive R-KV Diversity Calculation
// ============================================================================
#ifdef SDPA_STAGE_3

REQD_SUB_GROUP_SIZE(SUBGROUP_SIZE)
KERNEL(pa_sdpa_diversity)(
    OPTIONAL_SHAPE_INFO_ARG
    ...
    for (int token_offset = 0; token_offset < evictable_size; token_offset++) {
        ...
        const uint key_offset = block_indice * K_HEAD_SIZE * HEADS_NUM * PAGED_ATTENTION_BLOCK_SIZE +
                               head_idx * K_HEAD_SIZE * PAGED_ATTENTION_BLOCK_SIZE +
                               token_in_block;
        
        // Compute L2 norm
        SOFTMAX_ACCUMULATOR_TYPE norm_sq = 0.0f;
        for (int h = 0; h < K_HEAD_SIZE; h++) {
        ...
        const uint norm_key_offset = (seq_idx * HEADS_NUM * evictable_size * K_HEAD_SIZE) +
                                     (head_idx * evictable_size * K_HEAD_SIZE) +
                                     (token_offset * K_HEAD_SIZE);
    ...
    for (int i = get_local_id(2); i < evictable_size; i += get_local_size(2)) {
        for (int j = 0; j < evictable_size; j++) {
            const uint key1_offset = (seq_idx * HEADS_NUM * evictable_size * K_HEAD_SIZE) +
                                     (head_idx * evictable_size * K_HEAD_SIZE) +
                                     (i * K_HEAD_SIZE);
            const uint key2_offset = (seq_idx * HEADS_NUM * evictable_size * K_HEAD_SIZE) +
                                     (head_idx * evictable_size * K_HEAD_SIZE) +
                                     (j * K_HEAD_SIZE);
    ...
    if (head_idx > 0) {
        for (int i = get_local_id(2); i < evictable_size * evictable_size; i += get_local_size(2)) {
            atomic_add_global(&slm_aggregated[i], slm_cos_sim[i] / HEADS_NUM);
        }
    }
