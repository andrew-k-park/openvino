...
ConvertStridedSlicesToVariadicSplit::ConvertStridedSlicesToVariadicSplit() {
    add_matcher<ConvertFCStridedSlicesToVariadicSplitMatcher>();
    add_matcher<ConvertReshapeStridedSlicesToVariadicSplitMatcher>();
}

ConvertFCStridedSlicesToVariadicSplitMatcher::ConvertFCStridedSlicesToVariadicSplitMatcher() {
    ...
    auto m = std::make_shared<ov::pass::pattern::Matcher>(fully_connected_compressed_m, "ConvertFCStridedSlicesToVariadicSplitMatcher");
    this->register_matcher(m, callback);
}

ConvertReshapeStridedSlicesToVariadicSplitMatcher::ConvertReshapeStridedSlicesToVariadicSplitMatcher() {
    using namespace ov::pass::pattern;

    auto reshape_predicate = [](const ov::Output<ov::Node>& output) -> bool {
        const size_t num_users_to_fuse = 2;
        const auto reshape = ov::as_type_ptr<ov::op::v1::Reshape>(output.get_node_shared_ptr());
        size_t user_count = 0;
        for (const auto& user : reshape ->get_users()) {
            const auto strided_slice = ov::as_type_ptr<ov::op::v1::StridedSlice>(user);
            if (!strided_slice)
                return false;
            user_count++;
    ...
    auto reshape_m = wrap_type<ov::op::v1::Reshape>({any_input(), any_input()}, reshape_predicate);

    ov::matcher_pass_callback callback = [=](Matcher& m) {
        if (transformation_callback(m.get_match_root())) {
            return false;
        }
        auto reshape = ov::as_type_ptr<ov::op::v1::Reshape>(m.get_match_root());
        ov::NodeVector strided_slice_nodes;
        std::vector<int64_t> split_lengths;
        int64_t begin_offset = 0;
        int64_t end_offset = 0;
        ...
        auto name = reshape->get_friendly_name() + "_split";
        auto axis_const = ov::op::v0::Constant::create(ov::element::i64, ov::Shape{1}, {reshape->get_output_partial_shape(0).rank().get_length()- 1});
        auto split_lenghts_const = ov::op::v0::Constant::create(ov::element::i64, ov::Shape{2}, split_lengths);
        auto variadic_split = std::make_shared<ov::op::v1::VariadicSplit>(reshape, axis_const, split_lenghts_const);
        variadic_split->set_friendly_name(name);
        ov::copy_runtime_info(strided_slice_nodes, variadic_split);
    ...
    auto m = std::make_shared<ov::pass::pattern::Matcher>(reshape_m, "ConvertReshapeStridedSlicesToVariadicSplitMatcher");
    this->register_matcher(m, callback);
}
