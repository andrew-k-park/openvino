...
KVCache::KVCache(const Output<Node>& past,
                 ...
                 const std::vector<int64_t>& order_present,
                 const ov::element::Type output_type)
    : KVCache({past, new_token_data, beam_idx}, past_variable, true, concat_axis, gather_axis, order_present, output_type) {
    if (m_indirect)
        set_output_size(2);
...
KVCache::KVCache(const Output<Node>& past,
                 ...
                 const std::vector<int64_t>& order_present,
                 const ov::element::Type output_type)
    : KVCache({past, new_token_data}, past_variable, false, concat_axis, 0, order_present, output_type) {
    m_variable = past_variable;
    validate_and_infer_types();
}
...
KVCacheCompressed::KVCacheCompressed(const OutputVector& inputs,
                                     ...
                                     const std::vector<int64_t>& order_present,
                                     const ov::element::Type output_type)
    : KVCache(inputs, past_variable, true, concat_axis, gather_axis, order_present, output_type)
    , m_compressed(true)
...
std::vector<ov::PartialShape> shape_infer(const KVCacheCompressed* op,
                                          const std::vector<ov::PartialShape>& input_shapes,
                                          const std::vector<int64_t>& order_present) {
    std::vector<ov::PartialShape> out_shapes = shape_infer(static_cast<const KVCache*>(op), input_shapes, order_present);
    ...
    if (op->get_output_size() >= 3) {
        ...
        auto quantized_data_shapes =
            ov::op::internal::DynamicQuantize::shape_infer(&dq_op, { shape_present_t });
