...
KERNEL(fc)(
    ...
    uint out_b = ((batch_mega_block * DISPATCH_BSV + batch_mini_block) * TILE_B1);
    ...
    {
        INPUT0_TYPE tmp_input = input[input_offset + get_sub_group_local_id() % TILE_B1 * TILE_IN_B_PITCH];
...
#define CONST_LOOP_1(macro)
        ...
        // if (/* bi + out_b < OUTPUT_BATCH_NUM */ /* bi >= TILE_B1 */ false)
        #define LOAD_IN_0(bi) do {                                 \
                in_0[bi] = INPUT_BLOCK_READ(input, input_offset);     \
                input_offset += TILE_IN_B_PITCH; \
            } while (false)

            // if (TILE_B1 == 8) { CONST_LOOP_8(LOAD_IN_0) }
            // else if (TILE_B1 == 7) { CONST_LOOP_7(LOAD_IN_0) }
            // else if (TILE_B1 == 6) { CONST_LOOP_6(LOAD_IN_0) }
            // else if (TILE_B1 == 5) { CONST_LOOP_5(LOAD_IN_0) }
            // else if (TILE_B1 == 4) { CONST_LOOP_4(LOAD_IN_0) }
            // else if (TILE_B1 == 3) { CONST_LOOP_3(LOAD_IN_0) }
            // else if (TILE_B1 == 2) { CONST_LOOP_2(LOAD_IN_0) }
        ...
        // input_offset -= (TILE_B - TILE_B1) * TILE_IN_B_PITCH;
        // input_offset += TILE_IFM * SIMD - TILE_IN_B_PITCH * TILE_B1;
    ...
    #define LEFTOVER_IFM               (MAIN_LOOP_ELEMENTS_COUNT % (TILE_IFM * SIMD))
    {
        ...
        unroll_for(uint ki = 0; ki < CEIL_DIV(LEFTOVER_IFM, TILE_K); ++ki) {
            ...
            unroll_for (uint kii = 0; kii < TILE_K; ++kii) {
                unroll_for (uint fi = 0; fi < TILE_OFM; ++fi) {
                    #define MUL_OP2(bi) if (bi < TILE_B1) { \
                            const uint total_k = ki * TILE_K + kii; \
                    ...
                    // unroll_for (uint bi = 0; bi < TILE_B1; ++bi) {
                    //     const uint total_k = ki * TILE_K + kii;
                    //     if (total_k < LEFTOVER_IFM) {
                    //         INPUT0_TYPE in_val = _sub_group_shuffle(((INPUT0_TYPE*)(&in_0[bi]))[total_k / SIMD], total_k % SIMD);
                    //         ((ACCUMULATOR_TYPE*)(&acc[bi]))[fi] += in_val * ((FILTER_TYPE*)(&wei))[kii * TILE_OFM + fi];
    ...
    if (TILE_B1 != TILE_B) {
        // for (int bi = 0; bi < TILE_B1; bi++) {
#define WRITE_OP(bi) if (bi < TILE_B1) {OUTPUT_BLOCK_WRITE(output, output_offset, result[bi]); output_offset += TILE_OUT_B_PITCH;}
            // OUTPUT_BLOCK_WRITE(output, output_offset, result[bi]);
            // output_offset += TILE_OUT_B_PITCH;

            // if (TILE_B1 == 8) { CONST_LOOP_8(WRITE_OP) }
            // else if (TILE_B1 == 7) { CONST_LOOP_7(WRITE_OP) }
            // else if (TILE_B1 == 6) { CONST_LOOP_6(WRITE_OP) }
            // else if (TILE_B1 == 5) { CONST_LOOP_5(WRITE_OP) }
            // else if (TILE_B1 == 4) { CONST_LOOP_4(WRITE_OP) }
            // else if (TILE_B1 == 3) { CONST_LOOP_3(WRITE_OP) }
            // else if (TILE_B1 == 2) { CONST_LOOP_2(WRITE_OP) }
...
#undef WRITE_OP
        ...
        #define WRITE_OUTPUT(bi) do {                                       \
                if (/* bi + out_b < BATCH_SIZE */ bi >= TILE_B1)                          \
                    break; \
                    OUTPUT_BLOCK_WRITE(output, output_offset, result[bi]);  \
                output_offset += TILE_OUT_B_PITCH;                          \
            } while (false)
