...
// Default vector width in elements.  16 bytes per transaction:
//   fp16 -> 8,  fp32/int32 -> 4,  int8/uint8 -> 16,  int64 -> 2
...
JitConstants PermuteKernel_b_f_axes::GetJitConstants(const permute_params& params,
    ...
    const size_t x_tiles   = x_size / vec_width;      // full tiles
    const size_t x_rem     = x_size % vec_width;       // remainder elements
    ...
    if (!params.fused_ops.empty()) {
        // Fused ops need per-element scalar application inside the kernel.
        // Coordinate names used in the kernel: b, f, [w,] [z,] y, x (scalar x within tile)
...
CommonDispatchData PermuteKernel_b_f_axes::SetDefault(const permute_params& params) const {
    ...
    const size_t vec_width = GetVecWidth(params);
    // +1 tile for the remainder (handled by scalar path inside the kernel)
    ...
    // F is looped inside the kernel, not dispatched via GWS.
    // GWS[2] = B only, so consecutive work groups stream through
    // input memory sequentially, maximising DRAM burst utilisation.
...
bool PermuteKernel_b_f_axes::Validate(const Params& p) const {
    ...
    // Accept only simple (non-blocked) layouts
    ...
    // Target permutation: [1, 0, 2, ...] — swap B and F, keep spatial axes in order
    const auto& order = params.order;
    const size_t ndim = order.size();
    if (ndim < 3 || ndim > 6)
        DO_NOT_USE_THIS_KERNEL(p.layerID);

    // cldnn internal order is [b, f, x, y, z, w]
    // order[0] == 1 (b takes what was f) and order[1] == 0 (f takes what was b)
...
KernelsPriority PermuteKernel_b_f_axes::GetKernelsPriority(const Params& params) const {
    ...
    // Give higher priority when X is a clean multiple of the vector width
