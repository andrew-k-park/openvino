...
public:
    ...
    struct Config {
        ...
        bool is_ltx_video = false;      // ltx-video specific 3D spatial-temporal RoPE with pre-computed cos/sin from
                                        // 3D grid (frames, height, width). Uses complex interleaved format [batch, seq, 2048]
